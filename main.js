/* ═══════════════════════════════════════════════════════════════════
   INTELLIX 2026 — MAIN JS (ES Module)
   DarkVeil WebGL shader · GSAP animations · Card tilt
   ═══════════════════════════════════════════════════════════════════ */
...
vec3 hueShiftRGB(vec3 col,float deg){
  vec3 yiq=rgb2yiq*col;float rad=radians(deg);float cosh=cos(rad),sinh=sin(rad);
...
async function initDarkVeil() {
  ...
  try {
    ...
    console.warn('DarkVeil: OGL failed to load, using CSS fallback.', e);
...
function initAnimations() {
  ...
  if (typeof gsap === 'undefined' || typeof ScrollTrigger === 'undefined') {
    allAnimated.forEach(el => { el.style.opacity = '1'; el.style.transform = 'none'; });
    return;
  }
  ...
  /* Hero entrance */
  const heroEls = document.querySelectorAll('.hero__content [data-animate], .hero__scroll');
  const tl = gsap.timeline({ defaults: { ease: 'power3.out', duration: 0.9 } });

  heroEls.forEach(el => {
    const dir   = el.dataset.animate || 'fade-up';
    const delay = parseFloat(el.dataset.delay) || 0;
    const from  = { opacity: 0 };
    const to    = { opacity: 1 };
    if (dir === 'fade-up')   { from.y = 30;  to.y = 0; }
    if (dir === 'fade-down') { from.y = -20; to.y = 0; }
  ...
  document.querySelectorAll('a[href^="#"]').forEach(link => {
    link.addEventListener('click', e => {
      ...
      if (!target) return;
      e.preventDefault();
      // Offset by ~80px to accommodate fixed PillNav
...
function initPillNav() {
  ...
  function layout() {
    pills.forEach((pill) => {
      ...
      if (timelines.has(pill)) timelines.get(pill).kill();
  ...
  // Optional: Init anim effect
  const container = document.querySelector('.pill-nav-items');
  if (container) {
...
function initCubesBackground() {
  ...
  if (!scene || !registerSection) return;
  ...
  function resetAll() {
    cubes.forEach(cube => gsap.to(cube, { duration: leaveDur, rotateX: 0, rotateY: 0, ease: 'power3.out' }));
  }

  // Auto animate simulation
  ...
  registerSection.addEventListener('pointermove', (e) => {
    userActive = true;
  ...
  registerSection.addEventListener('pointerleave', resetAll);
